...
axios
  ...
  .catch((error) => {
    console.error(
      `❌ Error fetching tasks by category ${category}:`,
      error.response.data
    );
  });
```
...
axios
  ...
  .catch((error) => {
    console.error(
      `❌ Error fetching tasks based on due date:`,
      error.response.data
    );
  });
```
...
- `status` should only one of these [ ”`Pending`”, ”In `Progress`”, ”`Completed`” ]
- if Task `status` is already completed then you can't modify the status of task, it throws the error
